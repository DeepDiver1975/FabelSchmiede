...
export function buildServer(
  ...
  app.post<{ Body: { name: string; premise: string } }>(
    ...
    async (req, reply) => {
      ...
      // Create only generates the world plan — NOT the opening. The opening is
      // generated by /begin once the party is set up, so the GM can address the
      // real characters and combat has HP to work with. A campaign with zero
      // turns is "in setup"; the client routes it to the setup screen.
      let plan;
      try {
  ...
  // Begin the adventure: generate the opening scene now that the party exists.
  // Gated on a set-up party (>=1 character, each with Max HP) so the opening
  // knows who it addresses and combat is playable. Idempotent-guarded: a
  // campaign that already has turns has begun and cannot begin twice.
  app.post<{ Params: { id: string } }>("/api/campaigns/:id/begin", async (req, reply) => {
    const campaign = store.getCampaign(req.params.id);
    if (!campaign) return reply.code(404).send({ error: "Kampagne nicht gefunden." });
    if (campaign.status === "finished")
      return reply.code(409).send({ error: "Diese Kampagne ist abgeschlossen." });
    if (store.getTurns(req.params.id).length > 0)
      return reply.code(409).send({ error: "Diese Kampagne wurde bereits begonnen." });
    const characters = store.listCharacters(req.params.id);
    if (characters.length === 0 || characters.some((c) => (c.maxHp ?? 0) <= 0))
      return reply
        .code(400)
        .send({ error: "Bitte zuerst die Gruppe einrichten — jede Figur braucht Max. TP." });
    const plan = store.getPlan(req.params.id)?.plan;
    let opening;
    try {
      opening = await generateOpening(campaign.premise, call, characters, plan);
    } catch (err) {
      reply.log.error({ err, campaignId: req.params.id }, "opening generation failed (verhaspelt)");
      return reply.code(500).send({ error: VERHASPELT });
    }
    store.appendTurn(req.params.id, {
      role: "gm",
