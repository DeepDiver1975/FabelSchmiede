...
async function createCampaign(app: ReturnType<typeof setup>["app"]) {
  ...
  // Campaign creation is now plan-only; the opening is generated by /begin once
  // the party is set up. Most tests want a *started* campaign, so set up one
  // character with Max HP and begin — mirroring the two-step client flow.
  await app.inject({
    method: "POST",
    url: `/api/campaigns/${created.campaign.id}/characters`,
    payload: { name: "Held", concept: "Krieger", maxHp: 10 },
  });
  const begun = await app.inject({ method: "POST", url: `/api/campaigns/${created.campaign.id}/begin` });
  return begun.json();
}

// Create a campaign WITHOUT beginning it (setup phase, zero turns).
async function createCampaignShell(app: ReturnType<typeof setup>["app"]) {
  const res = await app.inject({
    method: "POST",
    url: "/api/campaigns",
    payload: { name: "Die Höhle", premise: "Goblins im Nebelwald" },
...
describe("server", () => {
  it("POST /api/campaigns creates a campaign in setup (plan only, no opening turn yet)", async () => {
    const { app } = setup();
    const body = await createCampaignShell(app);
    expect(body.campaign.name).toBe("Die Höhle");
    expect(body.campaign.status).toBe("active");
    expect(body.turns).toHaveLength(0); // no opening until /begin
    expect(body.brief.title).toBe("Die Höhle"); // plan/brief already available
    await app.close();
  });

  it("createCampaign helper begins the campaign, yielding a single opening gm turn", async () => {
    const { app } = setup();
  ...
  it("POST /begin rejects a campaign with no characters (400)", async () => {
    const { app } = setup();
    const { campaign } = await createCampaignShell(app);
    const res = await app.inject({ method: "POST", url: `/api/campaigns/${campaign.id}/begin` });
    expect(res.statusCode).toBe(400);
    await app.close();
  });

  it("POST /begin rejects when a character has no Max HP (400)", async () => {
    const { app } = setup();
    const { campaign } = await createCampaignShell(app);
    await app.inject({
      method: "POST",
      url: `/api/campaigns/${campaign.id}/characters`,
      payload: { name: "Ohne HP", concept: "Späher" },
    });
    const res = await app.inject({ method: "POST", url: `/api/campaigns/${campaign.id}/begin` });
    expect(res.statusCode).toBe(400);
    await app.close();
  ...
  it("POST /begin generates the opening with the party and cannot begin twice", async () => {
    const seen: string[] = [];
    ...
    expect(body.turns).toHaveLength(1);
    expect(body.turns[0].role).toBe("gm");
    // the opening prompt saw the party member
    expect(seen.some((s) => s.includes("Eröffnungsszene") && s.includes("Thalia"))).toBe(true);
    // begin is idempotent-guarded: a second begin is a 409
    const again = await app.inject({ method: "POST", url: `/api/campaigns/${campaign.id}/begin` });
